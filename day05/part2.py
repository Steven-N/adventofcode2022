...
def compute(data):
    ...
    for move in moves.splitlines():
        num_crates_to_move = int(re.search(pattern_move, move)[0].split()[1])
        ...
        for _ in range(num_crates_to_move):
            if len(stacks[stack_start]) > 0:
                crates_to_move.append(stacks[stack_start].pop(0))

        stacks[stack_end] = crates_to_move + stacks[stack_end]
...
@pytest.mark.parametrize(("test_input,expected"), [(INPUT, "MCD")])
